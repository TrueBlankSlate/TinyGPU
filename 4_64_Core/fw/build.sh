...
# build/02_import_sources.tcl). Do not hand-edit those files.
#
...
echo "   byte-identical to I_VLE64/I_VMACC/I_VADD/I_VSE64 in sim/tb_cva6_boot.v =="
"$OBJDUMP" -d "$HERE/$ELF"
...
if [ "$TARGET" = "bench" ]; then
    ...
    echo "  2. Run sim/tb_bench_image.v (in Vivado) -- it \$readmemh's the"
    ...
    echo "  2. Run sim/tb_boot_image.v (in Vivado) -- it \$readmemh's the"
