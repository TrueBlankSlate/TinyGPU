...
// vse64.v (CV-X-IF commit), not by polling memory for the write to land --
// an earlier version DID poll a TinyGPU-written address from software and
// hung forever (CVA6's dcache has no coherency with TinyGPU's separate AXI
// write master: the first poll read caches the stale value and never sees
// the update). In practice the AXI write burst's protocol completion
// (aw+16 beats+b_valid) IS still inside the measured window regardless,
// because CV-X-IF's issue_ready = (FSM state == IDLE) blocks CVA6 from
// issuing the *next* offloaded instruction until the FSM cycles all the
// way back to IDLE, which only happens after WRITEBACK's b_valid. Only the
// explicit software re-read of the written value was the hazard, not the
// burst's own completion -- see bench_matmul.S's header comment.
//
...
module tb_bench_image;
  ...
  // Relative filename -- same convention as tb_boot_image.v / axi4_bram_slave.v's
  // own INIT_FILE default. Vivado copies bench_image.hex (added via
  // import_sources.tcl as a "Memory Initialization File") into the sim
  // snapshot dir next to the compiled sources, so xsim resolves this
  // without needing an absolute, clone-location-specific path.
  localparam INIT_FILE = "bench_image.hex";
  ...
  initial begin
    ...
    // it parks -- wait for it to become non-zero, then a further margin
  ...
  initial begin
    ...
    $display("Either this isn't a bench build (use fw/build.sh bench, not the");
    $display("default boot_matmul.S build), or something in the offload");
    $display("sequence itself never reached WAIT_COMMIT/IDLE -- check the FSM.");
    $finish;
  end
